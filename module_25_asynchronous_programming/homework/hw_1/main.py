...
async def get_cat(client: aiohttp.ClientSession, idx: int) -> bytes:
    async with client.get(URL) as response:
        ...
        await asyncio.to_thread(write_to_disk,result, idx) # вызвал функцию записи через asyncio.to_thread


def write_to_disk(content: bytes, id: int): # Убрал ассинхронность в самой функции записи
